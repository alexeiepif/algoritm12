...
def edit_dist(a, b):
    ...
    Для поиска расстояния редактирвания используются 2 способа:
    1) edit_dist_td;
    2) edit_dist_dp.

    Для восстановления используется функция restore.
    """
    def edit_dist_td(i, j):
        """
        Поиск расстояния редактирования

        Испольуется динамическое программирование сверху вниз
        """
        if matrix[i][j] == large:
            if i == 0:
                matrix[i][j] = j
    ...
    def edit_dist_bu():
        ...
        Испольуется динамическое программирование снизу вверх
        """
        matrix = []
    ...
    if matrix == edit2:
        ...
        print("Что-то не так в коде", file=sys.stderr())
        exit(1)
